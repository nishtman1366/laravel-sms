...
class Ghasedak implements SmsInterface
{
    ...
    /**
     * Ghasedak constructor.
     */
    public function __construct()
    {
        $this->apiKey = Config::get('sms.providers.Ghasedak.apiKey');
        $client = new Client([
            'base_uri' => 'http://api.ghasedak.io',
            'timeout' => 10.0,
        ]);
        $this->client = $client;
    }
    ...
    public function send(array $to, array $text, bool $isFlash = false): array
    {
        $data = [
            'json' => [
                ...
                'receptor' => implode(',', $to),
                'linenumber' => Config::get('sms.providers.Ghasedak.lineNumber'),
                'senddate' => null,
                'checkid' => null
            ],
            'headers' => [
        ...
        try {
            $result = $this->client->post('v2/sms/send/bulk', $data);
    ...
    public function delivery(int $referenceId): array
    {
        ...
        try {
            $result = $this->client->post('v2/sms/status', $data);
            $result = json_decode($result->getBody(), true);
